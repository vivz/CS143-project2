...
clean:
	rm -f bruinbase bruinbase.exe *.o *~ lex.sql.c SqlParser.tab.c SqlParser.tab.h test_leaf test_nonleaf test_index test_bindex test1.idx test_select test_unsorted.tbl xsmall.idx xsmall.tbl xlarge.idx xlarge.tbl small.idx small.tbl medium.idx medium.tbl large.idx large.tbl test_node test_unsorted test.idx test2.idx test.tbl nonleaf.txt difference.txt
